...
class TestSlashCommandParser:
    ...
    def test_parse_command_with_mixed_args(self):
        ...
        command, args = parser.parse_message(
            "/test path:tests/unit verbose options=--coverage"
        )
        assert command == "test"
        assert args == {
            "path": "tests/unit",
    ...
    def test_execute_command_with_template(self):
        ...
        result = parser.execute_command(
            "deploy", {"branch": "feature", "env": "staging"}
        )
    ...
    def test_should_execute_disabled_command(self):
        ...
        config = {
            "slash_commands": {"dangerous": {"template": "rm -rf /", "disabled": True}}
        }
        parser = SlashCommandParser(config)
    ...
    def test_get_target_session(self):
        ...
        config = {
            "slash_commands": {
                "monitor": {"template": "htop", "target_session": "monitoring"}
            }
        }
        parser = SlashCommandParser(config)
...
class TestExpandSlashCommand:
    ...
    def test_expand_valid_command(self, mock_click):
        ...
        config = {
            "slash_commands": {
                "build": {"template": "npm build {env}", "defaults": {"env": "dev"}}
            }
        }

        is_slash, expanded, session, delay = expand_slash_command(
            "/build env:prod", config, "laptop"
        )
    ...
    def test_expand_restricted_command(self, mock_click):
        ...
        config = {
            "slash_commands": {
                "restart": {"template": "sudo reboot", "allowed_devices": ["server"]}
            }
        }

        is_slash, expanded, session, delay = expand_slash_command(
            "/restart", config, "laptop"
        )
        assert is_slash is True
        assert expanded is None  # Not executed due to restriction
        assert session is None
        mock_click.echo.assert_called_with(
            "Command '/restart' not allowed for device 'laptop'"
        )
...
class TestCheckTriggerConditions:
    ...
    def test_no_conditions(self):
        ...
        config = {"slash_commands": {"simple": {"template": "echo hello"}}}
    ...
    def test_execute_once_condition(self):
        ...
        config = {
            "slash_commands": {
                "backup": {"template": "backup.sh", "execute_once": True}
            }
        }
...
class TestIntegration:
    ...
    def test_full_command_flow(self, mock_click):
        ...
        is_slash, expanded, session, delay = expand_slash_command(
            "/deploy path:/myapp service:api", config, "server"
        )
        assert is_slash is True
    ...
    def test_complex_argument_parsing(self):
        ...
        assert args == {"message": "Hello_World", "subject": "Test-123"}

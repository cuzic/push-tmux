...
from push_tmux.triggers import (
    TriggerPattern,
    check_triggers,
    process_trigger_actions
)
...
class TestTriggerPattern:
    ...
    def test_regex_pattern_with_groups(self):
        ...
        config = {
            "triggers": {
                "deploy_trigger": {
                    "match": {
                        "pattern": r"deploy (\w+) to (\w+)",
                        "regex": True
                    },
                    "action": {
                        "template": "deploy.sh {group1} {group2}"
                    }
                }
            }
        }
    ...
    def test_template_variable_expansion(self):
        ...
        with patch('push_tmux.triggers.datetime') as mock_datetime:
            ...
            mock_datetime.now.return_value.strftime.side_effect = ["2024-01-01", "12:00:00"]
...
class TestComplexPatterns:
    ...
    def test_named_capture_groups(self):
        ...
        assert len(actions) == 1
        assert actions[0][1]['command'] == "restart_handler.sh --target=database --server=production"

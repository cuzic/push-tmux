...
class TestSlashCommandFallback:
    ...
    def test_undefined_command_with_fallback_enabled(self):
        ...
        config = {
            ...
            "slash_commands": {"deploy": {"template": "echo deploy"}},
        }

        # 未定義のコマンド /login をテスト
        is_slash, expanded, session, delay = expand_slash_command(
            "/login", config, "test-device"
        )
    ...
    def test_undefined_command_with_no_settings(self):
        ...
        config = {"slash_commands": {"deploy": {"template": "echo deploy"}}}
    ...
    def test_defined_command_always_works(self):
        ...
        is_slash, expanded, session, delay = expand_slash_command(
            "/deploy", config_enabled, "test-device"
        )
        assert is_slash
        ...
        is_slash, expanded, session, delay = expand_slash_command(
            "/deploy", config_disabled, "test-device"
        )
        assert is_slash
        assert expanded == "echo deploy"
        assert delay is None
    ...
    def test_non_slash_message_always_normal(self):
        ...
        is_slash, expanded, session, delay = expand_slash_command(
            "hello world", config, "test-device"
        )
    ...
    def test_edge_cases(self):
        ...
        # 特殊文字を含むコマンド
        is_slash, expanded, session, delay = expand_slash_command(
            "/@mention", config, "test-device"
        )
        assert not is_slash

        is_slash, expanded, session, delay = expand_slash_command(
            "/#hashtag", config, "test-device"
        )
        assert not is_slash
    ...
    def test_command_with_arguments_fallback(self):
        ...
        # 引数付きの未定義コマンド
        is_slash, expanded, session, delay = expand_slash_command(
            "/login user:admin pass:secret", config, "test-device"
        )
        assert not is_slash
        assert delay is None

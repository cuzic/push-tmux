...
class TestAutoRouteMode:
        ...
        with patch.dict(os.environ, {"PUSHBULLET_TOKEN": "test_token"}):
            with patch("push_tmux.AsyncPushbullet") as mock_pb_class:
                ...
                with patch(
                    ...
                    with patch(
                        "push_tmux.AsyncPushbulletListener"
                    ...
                    assert (
                        "tmuxセッションあり" in result.output
                        or "tmuxセッションなし" in result.output
                    )
            ...
            with patch(
                ...
                async def on_push_auto_route(push):
                    ...
                    if result.returncode == 0:
                        await mock_send(
                            config, push["body"], device_name=target_device_name
                        )
                ...
                mock_send.assert_called_once_with(
                    config, 'echo "Test auto-route"', device_name="push-tmux"
                )
    ...
    async def test_auto_route_skip_missing_session(self):
        ...
        device_cache = {"dev3": "non-existent"}
            ...
            with patch(
                ...
                with patch("click.echo") as mock_echo:
                    ...
                    async def on_push_auto_route(push):
                        ...
                        if result.returncode != 0:
                            mock_echo(
                                f"[スキップ] {target_device_name}: tmuxセッションが存在しません"
                            )
                            return
                    ...
                    mock_echo.assert_called_with(
                        "[スキップ] non-existent: tmuxセッションが存在しません"
                    )
    ...
    async def test_auto_route_multiple_sessions(self):
        ...
        device_cache = {"dev1": "push-tmux", "dev2": "1on1-ver2"}

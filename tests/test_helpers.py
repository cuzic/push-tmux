...
def create_tmux_mock(
    existing_sessions=None, current_session="test-session", windows="0", panes="0"
):
    ...
    async def mock_exec(*args, **kwargs):
        if "has-session" in args:
            ...
            result.communicate = AsyncMock(
                return_value=(f"{current_session}\n".encode(), b"")
            )
            return result
...
def assert_send_keys_called(mock_subprocess, expected_target, expected_message):
    ...
    assert message_call[0][3] == expected_target, (
        f"Expected target {expected_target}, got {message_call[0][3]}"
    )
    assert message_call[0][4] == expected_message, (
        f"Expected message {expected_message}, got {message_call[0][4]}"
    )

...
@pytest.mark.skip(
    reason="PyPI asyncpushbulletパッケージに移行したため統合テストを無効化"
)
...
class TestLiveRouting:
    ...
    async def test_message_routing_with_mock_pushbullet(self):
        ...
        with patch(
            "tests.integration.test_live_routing.AsyncPushbullet"
        ) as mock_pb_class:
            mock_pb = AsyncMock()
            mock_pb.get_devices = AsyncMock(
                return_value=[

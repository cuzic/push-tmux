...
class TestDeviceMapping:
    ...
    async def test_device_mapping_with_existing_session(self, mock_subprocess):
        ...
        config = {"device_mapping": {"mobile-dev": "frontend"}}
    ...
    async def test_device_mapping_with_detailed_format(self, mock_subprocess):
        ...
        config = {
            "device_mapping": {
                "backend-api": {"session": "backend", "window": "2", "pane": "1"}
            }
        }
    ...
    async def test_device_mapping_with_first_defaults(self, mock_subprocess):
        ...
        mock_subprocess.side_effect = create_tmux_mock(
            existing_sessions=["test-session"]
        )
    ...
    async def test_device_mapping_with_missing_session(self, mock_subprocess):
        ...
        config = {
            "device_mapping": {"mobile-dev": "nonexistent"},
            "tmux": {"default_target_session": "fallback"},
        }
        ...
        with patch("push_tmux.tmux.click.echo") as mock_echo:
            ...
            # 警告メッセージが表示されたことを確認
            warning_calls = [
                call for call in mock_echo.call_args_list if "警告" in str(call)
            ]
    ...
    async def test_device_name_as_session_default(self, mock_subprocess):
        ...
        config = {"tmux": {"use_device_name_as_session": True}}
    ...
    async def test_priority_explicit_config_over_mapping(self, mock_subprocess):
        """マッピングが最優先される"""
        config = {
            "device_mapping": {"test-device": "mapped-session"},
            "tmux": {"default_target_session": "default-session"},
        }

        # mapped-sessionが存在する
        mock_subprocess.side_effect = create_tmux_mock(
            existing_sessions=["mapped-session"]
        )
    ...
    async def test_error_when_no_session_found(self, mock_subprocess):
        ...
        async def no_sessions_mock(*args, **kwargs):
            if "has-session" in args:
                ...
                result = MagicMock()
                result.returncode = 1
                result.communicate = AsyncMock(
                    return_value=(b"", b"error: no current session")
                )
                return result
            else:
                result = MagicMock()

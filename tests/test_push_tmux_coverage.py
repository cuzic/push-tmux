    ...
    async def test_send_to_tmux_current_session(self):
        ...
        with patch.dict(os.environ, {"TMUX": "/tmp/tmux-1000/default,12345,0"}):
            ...
            with patch(
                ...
                assert any(
                    "display-message" in str(call) for call in mock_exec.call_args_list
                )
    ...
    async def test_send_to_tmux_no_tmux_env(self):
        ...
        with patch.dict(os.environ, {}, clear=True):
            with patch("click.echo") as mock_echo:
                ...
                # エラーメッセージが表示された
                error_calls = [
                    call for call in mock_echo.call_args_list if "エラー" in str(call)
                ]
                assert len(error_calls) > 0
                assert any("tmuxセッション" in str(call) for call in error_calls)

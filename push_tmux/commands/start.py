...
@click.option(
    "--daemon", is_flag=True, help="デーモンモード（ファイル変更監視とホットリロード）"
)
@click.option(
    "--once", is_flag=True, help="一回限りのメッセージ待機（デーモンモードと排他）"
)
...
@click.option(
    ...
    type=float,
    default=1.0,
    help="ファイル変更監視間隔（秒、デーモンモード時のみ）",
)
@click.option(
    "--watch-files",
    multiple=True,
    help="監視するファイル（複数指定可能、デーモンモード時のみ）",
)
def start(
    device,
    all_devices,
    ...
    processed_args = _process_start_arguments(
        device, all_devices, auto_route, no_auto_route
    )
    config = load_config()

    _execute_start_mode(
        config, processed_args, daemon, once, debug, reload_interval, watch_files
    )
...
def _process_start_arguments(device, all_devices, auto_route, no_auto_route):
    ...
    return {"device": device, "all_devices": all_devices, "auto_route": auto_route}


def _execute_start_mode(
    config, processed_args, daemon, once, debug, reload_interval, watch_files
):
    """適切なモードでstart処理を実行"""
    if daemon:
        ...
        _run_daemon_mode(
            config,
            processed_args["device"],
            processed_args["all_devices"],
            processed_args["auto_route"],
            debug,
            reload_interval,
            watch_files,
        )
...
def _run_daemon_mode(
    config, device, all_devices, auto_route, debug, reload_interval, watch_files
):
    """デーモンモード - ファイル変更監視付き"""
    ...
    _run_daemon_main_loop(
        file_timestamps, daemon_settings, device, all_devices, auto_route, debug
    )
...
def _prepare_daemon_settings(config, reload_interval, watch_files):
    ...
    actual_reload_interval = (
        reload_interval
        if reload_interval != 1.0
        else daemon_config.get("reload_interval", 1.0)
    )
...
def _run_daemon_main_loop(
    ...
    try:
        while True:
            reload_needed = _check_file_changes(
                file_timestamps, daemon_settings["watch_files"]
            )
...
def _check_file_changes(file_timestamps, watch_files):
    ...
    for filepath in watch_files:
        if Path(filepath).exists():
            ...
            if (
                filepath not in file_timestamps
                or current_mtime > file_timestamps[filepath]
            ):

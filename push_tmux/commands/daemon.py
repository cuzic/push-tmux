...
@click.option(
    "--reload-interval", type=float, default=1.0, help="ファイル変更監視間隔（秒）"
)
@click.option("--watch-files", multiple=True, help="監視するファイル（複数指定可能）")
def daemon(
    device, all_devices, auto_route, no_auto_route, debug, reload_interval, watch_files
):
    """
    ...
    mode_desc = (
        "自動ルーティング"
        if daemon_args["auto_route"]
        else (daemon_args["device"] or "default")
    )
    log_daemon_event(
        "info",
        "デーモンモードを開始",
        mode=mode_desc,
        auto_route=daemon_args["auto_route"],
        watch_files=watch_config["files"],
        reload_interval=watch_config["interval"],
    )

    if not _check_watchdog_available():
        run_simple_daemon(
            daemon_args["device"],
            daemon_args["all_devices"],
            daemon_args["auto_route"],
            daemon_args["debug"],
        )
        return
...
def _setup_watch_config(config, reload_interval, watch_files):
    ...
    return {"files": actual_watch_files, "interval": actual_reload_interval}
...
def _check_watchdog_available():
    ...
    try:
        ...
        log_daemon_event(
            "warning", "watchdogが利用できません。ファイル監視機能なしで実行します。"
        )
        return False
...
def _run_daemon_loop(handler, reload_interval):
    ...
    while True:
        ...
        if handler.process and handler.process.poll() is not None:
            log_daemon_event(
                "warning", "ワーカープロセスが予期せず終了しました。再起動します。"
            )
            handler.start_worker()
